...
export default function BrowserUseSettingsTab() {
  ...
  useEffect(() => {
    ...
    void loadSettings()
      .catch((err) => setError(err instanceof Error ? err.message : t('browser.loadError')))
      .finally(() => setIsSettingsLoading(false));

    void loadStatus()
      .catch((err) => setError(err instanceof Error ? err.message : t('browser.statusError')))
  ...
  const updateSettings = async (nextSettings: Partial<BrowserUseSettings>) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t('browser.saveError'));
    } finally {
  ...
  const installBrowserBinaries = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t('browser.installError'));
    } finally {
      setIsStatusLoading(false);
  ...
  const statusLabel = () => {
    if (isStatusLoading && !status) return t('browser.checking');
    if (status?.available) return t('browser.ready');
    if (browserEnabled) return t('browser.setupRequired');
    return t('browser.disabled');
  };
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div className="min-w-0 space-y-1">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {status?.message || t('browser.runtimeRequiredHint')}
                  </p>
                </div>
