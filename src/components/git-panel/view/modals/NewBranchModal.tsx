...
export default function NewBranchModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        <div className="p-6">
          <h3 id="new-branch-title" className="mb-4 text-lg font-semibold text-foreground">{t('git:newBranchModal.title')}</h3>

          <div className="mb-4">
            <label htmlFor="git-new-branch-name" className="mb-2 block text-sm font-medium text-foreground/80">
              {t('git:newBranchModal.branchName')}
            </label>
            <input
              ...
              placeholder={t('git:newBranchModal.placeholder')}
          ...
          <p className="mb-4 text-sm text-muted-foreground">
            {t('git:newBranchModal.description', { branch: currentBranch })}
          </p>
            ...
            >
              {isCreatingBranch ? (
                <>
                  ...
                  <span>{t('git:newBranchModal.creating')}</span>
                </>
              ) : (
                <>
                  <Plus className="h-3 w-3" />
                  <span>{t('git:newBranchModal.create')}</span>
                </>
              )}
            </button>
          </div>
