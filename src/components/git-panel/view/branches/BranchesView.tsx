      ...
      <div className="flex min-w-0 flex-1 flex-col gap-0.5">
        ...
        {isCurrent && (aheadCount > 0 || behindCount > 0) && (
          <div className="flex items-center gap-2 text-xs">
            {aheadCount > 0 && (
              <span className="text-green-600 dark:text-green-400">↑{aheadCount} {t('git:branches.ahead')}</span>
            )}
            {behindCount > 0 && (
              <span className="text-primary">↓{behindCount} {t('git:branches.behind')}</span>
            )}
          </div>
        )}
      </div>
        ...
        {isCurrent ? (
          ...
          <>
            ...
            <button
              ...
              title={t('git:branches.delete', { name })}
            >
...
export default function BranchesView({
  ...
  const requestSwitch = (branch: string) => {
    onRequestConfirmation({
      ...
      message: t('git:confirmModal.switchBranch', { branch }),
  ...
  const requestDelete = (branch: string) => {
    onRequestConfirmation({
      type: 'deleteBranch',
      message: t('git:confirmModal.deleteBranch', { branch }),
      onConfirm: () => void onDeleteBranch(branch),
    });
  };

  const localRemoteText = t('git:branches.localRemoteCount', {
    local: localBranches.length,
    remote: remoteBranches.length > 0 ? t('git:branches.remoteCount', { count: remoteBranches.length }) : '',
    ...
    <div className="flex flex-1 flex-col overflow-hidden">
      ...
      <div className="flex items-center gap-2 border-b border-border/40 px-4 py-2">
        ...
        <input
          ...
          placeholder={t('git:branches.searchPlaceholder')}
      ...
      <div className="flex-1 overflow-y-auto">
        {filteredLocalBranches.length > 0 && (
          <>
            <SectionHeader label={t('git:branches.local')} count={filteredLocalBranches.length} />
        ...
        {filteredRemoteBranches.length > 0 && (
          <>
            <SectionHeader label={t('git:branches.remote')} count={filteredRemoteBranches.length} />
        ...
        {filteredLocalBranches.length === 0 && filteredRemoteBranches.length === 0 && (
          <div className="flex h-32 flex-col items-center justify-center gap-2 text-muted-foreground">
            ...
            <p className="text-sm">{normalizedQuery ? t('git:branches.noBranchesMatch') : t('git:branches.noBranchesFound')}</p>
          </div>
        )}
      </div>
