...
export default function FileStatusLegend({ isMobile }: FileStatusLegendProps) {
  ...
  const LEGEND_ITEMS = [
    { status: 'M' as const, label: t('git:fileStatus.modified') },
    { status: 'A' as const, label: t('git:fileStatus.added') },
    { status: 'D' as const, label: t('git:fileStatus.deleted') },
    { status: 'U' as const, label: t('git:fileStatus.untracked') },
  ];
  ...
  return (
    <div className="border-b border-border/60">
      ...
      >
        ...
        <span>{t('git:fileStatus.legend')}</span>
