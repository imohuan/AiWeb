...
export default function CommitComposer({
  ...
  const requestCommitConfirmation = () => {
    ...
    onRequestConfirmation({
      type: 'commit',
      message: t('git:confirmModal.commitFiles', { count: selectedFileCount, message: trimmedMessage }),
      onConfirm: async () => {
      ...
      {isMobile && isCollapsed ? (
        <div className="border-b border-border/60 px-4 py-2">
          ...
          >
            ...
            <span>{t('git:changes.commitMobile', { count: selectedFileCount })}</span>
        ...
        <div className="border-b border-border/60 px-4 py-3">
          {isMobile && (
            <div className="mb-2 flex items-center justify-between">
              <span className="text-sm font-medium text-foreground">{t('git:changes.commitChanges')}</span>
              <button
          ...
          <div className="relative">
            ...
            <div className="absolute right-2 top-2 flex gap-1">
              <button
                ...
                title={t('git:changes.generateCommitMessage')}
              >
                {isGeneratingMessage ? (
          ...
          <div className="mt-2 flex items-center justify-between">
            <span className="text-sm text-muted-foreground">
              {t('git:changes.filesSelected', { count: selectedFileCount })}
            </span>
            <button
            ...
            >
              ...
              <span>{isCommitting ? t('git:changes.committing') : t('git:changes.commit')}</span>
            </button>
          </div>
        </div>
      )}
